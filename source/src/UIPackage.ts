...
module fgui {
    ...
    export class UIPackage {
        ...
        public _dependencies: Array<PackageDependency>;
        ...
        public HasAssets(item: PackageItem): boolean {
            switch (item.type) {
                case PackageItemType.Image:
        ...
        public getItemAssetAsync(item: PackageItem, onComplete: Handler): void {
            if (item.IsDecoded()) {
        ...
        private loadImage(item: PackageItem): void {
            var sprite: AtlasSprite = this._sprites[item.id];
            if (sprite) {
                this.getItemAssetAsync(sprite.atlas, Handler.create(this, this._loadedImage, [item]))
            } else {
                let arr = item.loading;
                delete item.loading;
                if (arr) {
        ...
        private _loadedImage(srcItem: PackageItem, item: PackageItem) {
            var sprite: AtlasSprite = this._sprites[srcItem.id];
            var atlas: egret.Texture = item.asset as egret.Texture
            if (atlas) {
                srcItem.asset = new egret.Texture();
                srcItem.asset.bitmapData = atlas.bitmapData;
                srcItem.asset.$initData(atlas.$bitmapX + sprite.rect.x, atlas.$bitmapY + sprite.rect.y,
                    sprite.rect.width, sprite.rect.height,
                    sprite.offset.x, sprite.offset.y,
                    sprite.originalSize.x, sprite.originalSize.y,
                    atlas.$sourceWidth, atlas.$sourceHeight,
            ...
            let arr = srcItem.loading;
            delete srcItem.loading;
            if (arr) {
                arr.forEach(e => e.runWith(srcItem));
            }
        }

        private loadAtlas(item: PackageItem): void {
            RES.getResAsync(item.file, (obj, name) => {
                item.asset = obj
                let arr = item.loading;
                delete item.loading;
                arr.forEach(e => e.runWith(item));
